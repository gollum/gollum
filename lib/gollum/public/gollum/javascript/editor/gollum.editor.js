  ...
  $.GollumEditor = function( IncomingOptions ) {
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      ...
      if (ActiveOptions['section'] && $.markupSupportsEditableSections ( ActiveOptions.MarkupType )) {
        ...
        if (sectionLine) {
          var newCursorLine = sectionLine+1 //gotoLine starts counting at 1 instead of 0, so +1
          window.ace_editor.gotoLine(newCursorLine, window.ace_editor.session.getLine(newCursorLine).length, animate=false); 
