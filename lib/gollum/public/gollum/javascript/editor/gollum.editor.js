...
(function($) {
  ...
  **/
  $.GollumEditor.replaceSelection = function ( repText, selPos ) {
     if (typeof selPos === 'undefined') { selPos = $gollum_editor.getSelectionRange(); }
     if ( repText ) {
       $gollum_editor.session.replace(selPos, repText)
      }
  };
    ...
    setActiveLanguage: function( name ) {
      ...
      if ( !LanguageDefinition.isLoadedFor(name) ) {
        ...
        LanguageDefinition.loadFor( name, function(x, t) {
          ...
          $gollum_editor.commands.addCommand({
            name: 'h1',
   ...
   */
   var FunctionBar = {
      ...
      executeAction: function( definitionObject ) {
        var selPos = $gollum_editor.getSelectionRange();
        var selText = $gollum_editor.session.getTextRange(selPos);
        var repText = selText;
        var reselect = true;
        var cursor = null;
        ...
        $gollum_editor.selection.moveCursorToPosition(selPos);
        ...
        if ( reselect ) {
          ...
          $gollum_editor.selection.setRange(selPos);
          $gollum_editor.focus();
        }
   ...
   var FormatSelector = {
     ...
     evtChangeFormat: function( e ) {
       ...
       FormatSelector.setMarkupMode(newMarkup);
       $gollum_editor.focus();
     },
    ...
    updateSelected: function() {
       ...
       FormatSelector.setMarkupMode( currentLang );
