...
(function($) {
  ...
  var AsciiDoc = $.extend(true, {}, $.DefaultLang); // Use default language definition (markdown) as a base, and modify where necessary

  AsciiDoc['function-bold']['replace'] = "*$1*$2";
  AsciiDoc['function-hr'] = undefined;
  AsciiDoc['function-blockquote']['replace'] = "----\n$1$2\n----";

  // Set h1-h3 functions
  for (var i=1; i < 4; i++) {
    AsciiDoc['function-h' + i.toString()]['replace'] = "=".repeat(i) + " $1$2";
  }

  AsciiDoc['function-link']['replace'] = function ( res ) {
    var rep = '';
  ...
  AsciiDoc['function-image']['replace'] = function ( res ) {
    var rep = '';
    if ( res['url'] && res['alt'] ) {
